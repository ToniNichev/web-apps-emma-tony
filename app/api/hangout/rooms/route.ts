...
export async function POST(request: Request) {
  ...
  // Mutual follows only — same gate as the Kindness Wall / Draw & Guess.
  ...
  const [result] = await db.execute(
    'INSERT INTO hangout_rooms (created_by) VALUES (?)',
    [session.id]
  ) as any[];
  const roomId = (result as any).insertId;

  await db.execute(
    'INSERT INTO hangout_room_players (room_id, user_id, status, joined_at) VALUES (?, ?, "joined", NOW())',
    [roomId, session.id]
  );
