...
export async function GET() {
  ...
  const [rows] = await db.execute(
    `SELECT r.id, r.status, r.category, r.current_round, r.created_at,
...
export async function POST(request: Request) {
  ...
  const [result] = await db.execute(
    'INSERT INTO trivia_rooms (created_by) VALUES (?)',
    [session.id]
  ) as any[];
  const roomId = (result as any).insertId;

  await db.execute(
    'INSERT INTO trivia_room_players (room_id, user_id, status, joined_at) VALUES (?, ?, "joined", NOW())',
    [roomId, session.id]
  );

  for (const inviteeId of inviteeIds) {
    await db.execute(
      'INSERT INTO trivia_room_players (room_id, user_id, status) VALUES (?, ?, "invited")',
      [roomId, inviteeId]
    );
    await db.execute(
      'INSERT INTO notifications (user_id, actor_id, type, message_preview) VALUES (?, ?, "trivia_invite", ?)',
