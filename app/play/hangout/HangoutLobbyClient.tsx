...
export default function HangoutLobbyClient({ initialRooms, friends }: { initialRooms: Room[]; friends: Friend[] }) {
  ...
  async function createRoom() {
    ...
    setCreating(true);
    setError(null);
    const res = await fetch('/api/hangout/rooms', {
      method: 'POST',
    ...
    setCreating(false);
    if (res.ok) {
      router.push(`/play/hangout/${data.id}`);
    } else {
    ...
    <div className="space-y-5">
      {!picking ? (
        ...
        <div className="card p-5 space-y-4">
          ...
          <div className="flex gap-2">
            ...
            >
              {creating ? 'Creating…' : `Create room (${selected.size})`}
            </button>
          </div>
        </div>
      )}
      ...
      {rooms.length === 0 ? (
        <div className="card p-12 text-center">
          ...
          <p className="text-gray-400 font-medium">No hangout rooms yet</p>
        </div>
      ) : (
