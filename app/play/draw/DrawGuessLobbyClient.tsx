...
export default function DrawGuessLobbyClient({ initialRooms, friends }: { initialRooms: Room[]; friends: Friend[] }) {
  ...
  async function createRoom() {
    ...
    setCreating(true);
    setError(null);
    const res = await fetch('/api/games/draw/rooms', {
      method: 'POST',
    ...
    <div className="space-y-5">
      {!picking ? (
        ...
        <div className="card p-5 space-y-4">
          ...
          {friends.length === 0 ? (
            <p className="text-sm text-gray-400">Follow some friends who follow you back to invite them to play.</p>
          ) : (
          ...
          <div className="flex gap-2">
            ...
            >
              {creating ? 'Creating…' : `Create game (${selected.size})`}
            </button>
          </div>
        </div>
      )}
            ...
            >
              <div>
                <p className="text-sm font-semibold text-gray-700">
                  {r.host_first_name}&apos;s game
                  {r.my_status === 'invited' && <span className="ml-2 text-xs text-purple-500 font-bold">Invited</span>}
                </p>
                <p className="text-xs text-gray-400 mt-0.5">
                  {r.player_count} playing · {r.status === 'lobby' ? 'Waiting to start' : r.status === 'active' ? `Round ${r.current_round}` : 'Finished'}
                </p>
              </div>
